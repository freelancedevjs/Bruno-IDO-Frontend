...
import { getCountDownNumberArray, zeroPad } from "@helpers/formatNum";
import { NumberBox } from "@components/ComingSoon";
import useWidth from "@hooks/useWidth";

const TimerBox = ({ timeUnit, text }: { timeUnit: number; text: string }) => {
  return (
    <div>
      <div className="flex gap-1">
        <NumberBox value={getCountDownNumberArray(timeUnit)[0]} />
        <NumberBox value={getCountDownNumberArray(timeUnit)[1]} />
      </div>
      <p className="text-base sm:text-lg text-center font-normal mt-3">
        {text}
      </p>
    </div>
  );
...
export const CountdownTimer: React.FC<ICountdownTimer.IProps> = (
  ...
  switch (variant) {
    case "simple":
      return (
        <div
          className={`flex items-end text-white  text-xl font-semibold ${className}`}>
          <>
            {timeLeft.days > 0 && (
              <>
                ...
                <div className="text-base font-light ml-1">d</div>
                <div className="text-base font-light mx-1">:</div>
              </>
            )}
            <div className=" text-center text-primary">
              {zeroPad(timeLeft.hours)}
            </div>{" "}
            <div className="text-base font-light ml-1">h</div>
            <div className="mx-1">:</div>
            <div className=" text-center text-primary">
              {zeroPad(timeLeft.minutes)}
            </div>
            <div className="text-base font-light ml-1">m</div>
            <div className="mx-1">:</div>
            <div className=" text-center text-primary">
              {zeroPad(timeLeft.seconds)}
            </div>
            <div className="text-base font-light ml-1">s</div>
          </>
        </div>
      );
    case "card":
      return (
        <div
          className={`flex justify-center items-center text-base md:text-xl font-semibold ${className}`}>
          <>
            {timeLeft.days > 0 && (
              <>
    ...
    case "coming-soon":
      return (
        <div
          className={`flex  items-center flex-row justify-center mt-6 gap-3 sm:gap-6 text-white ${className}`}>
          <TimerBox timeUnit={timeLeft.days} text="Days" />
          <TimerBox timeUnit={timeLeft.hours} text="Hours" />
          <TimerBox timeUnit={timeLeft.minutes} text="Minutes" />
          {width > 640 && (
            <TimerBox timeUnit={timeLeft.seconds} text="Seconds" />
          )}
        </div>
      );
